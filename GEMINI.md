...
## Identity & Commits
- **Author Name:** Gemini
- **Author Email:** gemini@google.com
- **Commit Subject Prefix:** `(GEMINI)`
- **Commit Description:** ALWAYS provide a non-empty, detailed description of changes. Never leave the body blank.
...
- Run API locally: `uvicorn bt_servant_engine.api_factory:create_app --factory --reload`
...
- Auto-commit small prompt fixes:
  - For low-risk edits to prompt text/constants (e.g., `PASSAGE_SUMMARY_AGENT_SYSTEM_PROMPT` wording tweaks) or small doc updates, commit directly with a proper `(GEMINI)` message without asking for confirmation.
...
- Do not rely on brittle string scans or regexes to extract languages, intents, or options from user text
  (e.g., checking for phrases like "into <lang>", "in <lang>", or scanning for language names). Natural language is highly variable and ambiguous (code-switching, morphology, synonyms, typos, punctuation), which causes false positives/negatives and inconsistent behavior.
...
- Branch/state:
  - On `main` at latest commits:
    - (GEMINI) Add legacy init() shim for tests after moving to lifespan
    - (GEMINI) Fix indentation and import order after lifespan/exception refactor
    - (GEMINI) Replace startup event with lifespan; narrow exception handling to satisfy IDE warnings
    - (GEMINI) Finalize hooks setup: address mypy Optional brain in bt_servant
    - (GEMINI) Enable hooks and doc update; fix lint fallout for bt_servant.py and chroma imports
    - (GEMINI) Clean bt_servant.py: docstrings, typing, and safe exception handling
    - (GEMINI) Relax chroma types and embedder typing in db/chroma_db.py
    - (GEMINI) Clean messaging.py; add to enforced checks
    - (GEMINI) Clean db/user.py typing; add to enforced checks
    - (GEMINI) Rename helper to check_commit.sh; add CHECK_ALL support
    - (GEMINI) Rename helper to check_cleaned.sh and update hook/docs
    - (GEMINI) Expand incremental checks to include user_message.py
- Current guarantees:
...
- Pre-commit setup:
  ...
  - On Windows, run commits from Git Bash; no chmod needed. If `$''` errors appear, convert scripts to LF.
