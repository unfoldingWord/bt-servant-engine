...
Output: sources/bible_data/en/<book>.json
...
def main(argv: List[str]) -> int:
    ...
    # Optional first arg: custom output directory (defaults to ./sources/bible_data/en)
    out_dir = (
        Path(argv[1]) if len(argv) > 1 else Path("sources") / "bible_data" / "en"
    )
