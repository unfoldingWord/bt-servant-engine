...
from utils.bsb import BOOK_MAP as BSB_BOOK_MAP, normalize_book_name, select_verses, label_ranges, select_translation_challenges
from db import (
...
(4) If the GET_BIBLE_TRANSLATION_ASSISTANCE intent, the GET_PASSAGE_SUMMARY intent, or the GET_PASSAGE_TRANSLATION_CHALLENGES intent, was processed, the information contained in the response message generated by these intent processing nodes should usually be as close to the beginning as possible, unless that would violate guideline #1 above.
...
<intents>
  ...
  <intent name="get-passage-translation-challenges">
...
<examples>
  ...
  <example>
    ...
    <intent>get-passage-translation-challenges</intent>
  </example>
  <example>
...
class IntentType(str, Enum):
    ...
    GET_PASSAGE_TRANSLATION_CHALLENGES = "get-passage-translation-challenges"
...
def process_intents(state: Any) -> List[str]:
    ...
    if IntentType.GET_PASSAGE_TRANSLATION_CHALLENGES in user_intents:
        nodes_to_traverse.append("handle_get_passage_translation_challenges_node")
...
def handle_get_passage_translation_challenges(state: Any) -> dict:
    ...
    query = s["transformed_query"]
    query_lang = s["query_language"]
    logger.info("[passage-challenges] start; query_lang=%s; query=%s", query_lang, query)

    # Translate to English for parsing, if needed
    if query_lang == Language.ENGLISH.value:
        parse_input = query
        logger.info("[passage-challenges] parsing in English (no translation needed)")
    else:
        logger.info("[passage-challenges] translating query to English for parsing")
        parse_input = translate_text(query, target_language="en")
    ...
    logger.info("[passage-challenges] extracting passage selection via LLM")
    selection_resp = open_ai_client.responses.parse(
        model="gpt-4o",
        input=selection_messages,
        text_format=PassageSelection,
    ...
    selection: PassageSelection = selection_resp.output_parsed
    logger.info("[passage-challenges] extracted %d selection(s)", len(selection.selections))
    ...
    if not selection.selections:
        ...
        logger.info("[passage-challenges] no passage detected; prompting user for clearer reference")
    ...
    for sel in selection.selections:
        ...
        if canonical not in BSB_BOOK_MAP:
            ...
            logger.info("[passage-challenges] unsupported book requested: %s", sel.book)
            return {"responses": [{"intent": IntentType.GET_PASSAGE_TRANSLATION_CHALLENGES, "response": msg}]}
        canonical_books.append(canonical)
        normalized_selections.append(PassageRef(
            book=canonical,
    ...
    if len(set(canonical_books)) != 1:
        ...
        logger.info("[passage-challenges] cross-book selection detected; rejecting")
        return {"responses": [{"intent": IntentType.GET_PASSAGE_TRANSLATION_CHALLENGES, "response": msg}]}

    canonical_book = canonical_books[0]
    logger.info("[passage-challenges] canonical_book=%s", canonical_book)
    ...
    for sel in normalized_selections:
        if sel.start_chapter is None:
            ...
            ranges.append((sel.start_chapter, sel.start_verse, sel.end_chapter, sel.end_verse))
    logger.info("[passage-challenges] ranges=%s", ranges)

    # Retrieve translation challenges from verse_data JSONs
    data_root = Path("sources") / "verse_data"
    logger.info("[passage-challenges] retrieving challenges from %s", data_root)
    items = select_translation_challenges(data_root, canonical_book, ranges)
    logger.info("[passage-challenges] retrieved %d verse entries", len(items))
    if not items:
        msg = (
            "I couldn't locate any translation notes for that selection. Please check the reference and try again."
        )
        logger.info("[passage-challenges] no challenges found for selection; prompting user")
        return {"responses": [{"intent": IntentType.GET_PASSAGE_TRANSLATION_CHALLENGES, "response": msg}]}
    ...
    challenge_resp = open_ai_client.responses.create(
        model="gpt-5",
        reasoning=cast(Any, {"effort": "low"}),
        instructions=PASSAGE_TRANSLATION_CHALLENGES_AGENT_SYSTEM_PROMPT,
        input=sum_messages,
        store=False,
    )
    challenge_text = challenge_resp.output_text
    logger.info("[passage-challenges] challenges generated (len=%d)", len(challenge_text) if challenge_text else 0)

    response_text = f"Translation challenges for {ref_label}:\n\n{challenge_text}"
    logger.info("[passage-challenges] done")
    return {"responses": [{"intent": IntentType.GET_PASSAGE_TRANSLATION_CHALLENGES, "response": response_text}]}
...
def create_brain():
    ...
    builder.add_node("handle_get_passage_translation_challenges_node", handle_get_passage_translation_challenges)
    ...
    builder.add_edge("handle_get_passage_translation_challenges_node", "translate_responses_node")
